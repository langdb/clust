...
use langdb_clust::messages::{
    CacheControl, ClaudeModel, ContentBlock, MaxTokens, Message,
    MessagesRequestBody, SystemPrompt, TextContentBlock,
};
...
async fn main() -> anyhow::Result<()> {
    ...
    let system_prompt = SystemPrompt::from_text_blocks_with_cache_control(
        vec![
            (
    ...
    println!(
        "Sending request with advanced system prompt and cache control..."
    );
